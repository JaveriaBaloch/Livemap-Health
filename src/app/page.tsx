...
export default function Home() {
  ...
  const openDir = (s: Specialist, b: ActiveScreen = "dashboard", emId?: string) => { setDirT(s); setDirBack(b); setTrackingEmergencyId(emId || null); setScreen("directions"); };
  ...
  if (screen === "directions" && dirT) return <DirectionsMap specialist={dirT} emergencyId={trackingEmergencyId} onBack={() => { setTrackingEmergencyId(null); if (isMedical && accEm) setScreen("emergency-response"); else setScreen(dirBack); }} />;
  ...
  if (screen === "emergency-response" && accEm && isMedical) return <EmergencyResponse emergency={accEm} onBack={() => { setAccEm(null); setScreen("dashboard"); }} onGetDirections={openEmDir} />;
  if (isMedical) return <DoctorDashboard onAcceptEmergency={(e) => { setAccEm(e); setScreen("emergency-response"); }} onGetDirections={openEmDir} onViewAccepted={(e) => { setAccEm(e); setScreen("emergency-response"); }} onOpenProfile={() => setScreen("profile")} />;
  if ((user.role as string) !== "emergency-patient") return <PatientDashboard onNavigate={setScreen} onOpenProfile={() => setScreen("profile")} onTrackDoctor={(s, d) => { if (!d.location?.lat || !d.location?.lng) { alert("Doctor location not available yet."); return; } openDir({ place_id: `doc-${d.doctorId || Date.now()}`, name: `Dr. ${d.doctorName || "Doctor"}`, rating: 0, user_ratings_total: 0, vicinity: d.specialization || "En route", formatted_phone_number: d.phone, types: ["doctor"], geometry: { location: { lat: d.location.lat, lng: d.location.lng } }, formatted_distance: d.estimatedArrival || "Nearby" }, "dashboard", s.id); }} />;
